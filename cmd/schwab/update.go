...
func runUpdate(ctx context.Context) error {
	...
	staged, err := os.CreateTemp(filepath.Dir(exe), ".schwab-update-*")
	if err != nil {
		if errors.Is(err, os.ErrPermission) {
			return fmt.Errorf("%s is not writable, re-run with: sudo schwab update", filepath.Dir(exe))
		}
	...
	archiveName := fmt.Sprintf("schwab-%s-%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	...
	if err := extractBinaryFromTar(archive, "schwab", staged); err != nil {
	...
	fmt.Printf("Updated to %s. Run `schwab version` to confirm.\n", release.TagName)
	return nil
}
