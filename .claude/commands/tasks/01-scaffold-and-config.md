...
- README updated: install/dev instructions matching the spec's quickstart (`git clone`, `npm install`, `npm run dev`), and a short note on `REN_DATA_DIR`.
...
- [ ] Setting `REN_DATA_DIR=/some/path npm run dev` uses that path instead, creating `inbox/`, `knowledge/`, `runtime/` if absent.
