...
- **Backups/export**: `sia export` — a straightforward archive of `knowledge/` (+ optionally `inbox/`) to a single portable bundle. Not a sync service, just an export command.
- **`sia validate`**: run schema validation (Task 02) across the whole data dir and report problems, without needing a full reindex.
- **`sia doctor`**: sanity-check config, data dir permissions/structure, DB integrity, and (if configured) AI provider reachability.
- **Audit/change history**: decide whether Task 07's `changelog.jsonl` is enough or needs a small viewer/CLI (`sia log`) — don't build more than the spec's "maintain a change log where practical" asks for.
...
- `LICENSE`, `docs/deployment.md`, `sia export`/`validate`/`doctor` commands (as much as is genuinely useful — not all are required if the checklist above says otherwise).
- Updated `README.md` meeting the spec's open-source checklist.
- A closing summary of what was deferred beyond v1 and why, for whoever picks up Sia next.
